...
def user_error(message, exception, code=None, vars=None):
    ...
    logging.error(
        f"{colors.lightred}ERROR{colors.reset}\t{message}{': ' + colors.lightyellow + str(code) + colors.reset if code else ''}"
    )
    if vars and vars.get("input_row_number") and vars.get("_values"):
        logging.error(
            f"\tat data row #{vars['input_row_number']}:"
            f" {colors.darkgray}{vars['_values']}{colors.reset}"
        )
...
def user_warning(message, exception=None, code=None):
    ...
    logging.warning(
        f"{colors.lightyellow}WARNING{colors.reset}\t{message}{': ' + colors.lightyellow + str(code) + colors.reset if code else ''}"
    )
...
def user_info(message, code=None):
    ...
    """
    logging.info(
        f"{colors.green}INFO{colors.reset}\t{message}{': ' + colors.yellow + str(code) + colors.reset if code else ''}"
    )
...
def user_debug(message, code=None):
    ...
    """
    logging.debug(
        f"{colors.lightcyan}DEBUG{colors.reset}\t{message}{': ' + colors.lightgray + str(code) + colors.reset if code else ''}"
    )
